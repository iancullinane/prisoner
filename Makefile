...
test-integration:
	go test -tags=integration -count=1 -timeout=10m ./api/...
...
run-server-postgres: build compose-up
	...
	DATABASE_URL='postgres://prisoner:prisoner@127.0.0.1:5432/prisoner?sslmode=disable' ./bin/prisoner -server -store postgres
